...
def blast_predict(annotations, query_file, 
    ...
    # Load annotations from 3-column format
    if annotations.endswith('.tsv'):
        print("Loading annotations from TSV file")
        terms_df = pd.read_csv(annotations, sep='\t', header=None, names=['EntryID', 'term', 'aspect'])
        # terms_df = terms_df[terms_df['aspect'] == 'GO']
        annotation_mat, proteins, terms, _ = sparse_matrix_and_indices(terms_df)
...
def parse_args(argv):
    ...
    parser.add_argument('--annotations', '-a', 
                        help='Path to the propagated annotation sparse matrix or a .gaf or .dat file or a 3-column tsv', required=True)
